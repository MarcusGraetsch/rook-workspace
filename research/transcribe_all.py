...
PODCAST_DIR = RESEARCH_DIR / "podcasts"
TRANSCRIPT_DIR = RESEARCH_DIR / "podcasts" / "transcripts"

# Verzeichnisse erstellen
TRANSCRIPT_DIR.mkdir(parents=True, exist_ok=True)

# Alle MP3s finden
mp3_files = sorted(glob.glob(str(PODCAST_DIR / "*.mp3")))

print(f"🎙️ Starte Transkription von {len(mp3_files)} Episoden...")
print(f"   Transkripte werden gespeichert in: {TRANSCRIPT_DIR}")
print()
...
for i, mp3_path in enumerate(mp3_files, 1):
    ...
    # Überspringen wenn bereits transkribiert
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=3600  # 1 Stunde Timeout pro Episode
        )
