...
const DEFAULT_SCHEMA = path.resolve(__dirname, '../schemas/rook-hermes-event.schema.json');
...
async function readJson(filePath, label) {
  ...
  try {
    ...
    fail(`${label} not readable: ${filePath}: ${error.message}`);
  }
...
function validateEvent(schema, event) {
  ...
  if (typeof event.event_type !== 'string' || !/^[a-z][a-z0-9._-]*$/.test(event.event_type)) {
    fail('event_type must be lowercase and match ^[a-z][a-z0-9._-]*$');
  }

  if (typeof event.created_at !== 'string' || !ISO_DATE_TIME.test(event.created_at)) {
    fail('created_at must be an ISO-8601 UTC timestamp ending in Z');
  }
  ...
  if ('acks' in event) {
    if (!Array.isArray(event.acks)) {
      fail('acks must be an array when present');
    }
    for (const [index, ack] of event.acks.entries()) {
      ...
      if (typeof ack.acknowledged_at !== 'string' || !ISO_DATE_TIME.test(ack.acknowledged_at)) {
        fail(`acks[${index}].acknowledged_at must be an ISO-8601 UTC timestamp ending in Z`);
      }
      for (const key of Object.keys(ack)) {
        if (!['system', 'acknowledged_at', 'notes'].includes(key)) {
          fail(`unexpected acks[${index}] field: ${key}`);
        }
      }
    }
  }
  ...
  if ('delivery' in event) {
    ...
    if (typeof event.delivery.state !== 'string') {
      fail('delivery.state must be a string');
    }
    if (!Number.isInteger(event.delivery.attempts) || event.delivery.attempts < 0) {
      fail('delivery.attempts must be a non-negative integer');
    }
