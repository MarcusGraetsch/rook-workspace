...
- Archive cleanup is a guarded maintenance action. `operations/bin/plan-archive-task-cleanup.mjs` stays dry-run unless `--apply` is paired with `--task-id <id>` or `--allow-reviewed`. Apply mode only moves runtime archive duplicates, requires a fresh `/root/backups/rook-runtime/` snapshot, and writes a manifest next to each quarantined record under `/root/.openclaw/runtime/operations/archive/task-collisions/`.
- Git-backed historical task-id collisions are accepted only through hash-pinned manifests under `/root/.openclaw/workspace/operations/archive/task-collisions/`. If either task file changes, the manifest stops suppressing the integrity warning and cleanup action.
...
- Runtime posture exceptions should be explicit. If a warning is intentionally accepted, document it in `operations/config/runtime-posture-policy.json` with constraints, a reason, and a `review_after` date instead of leaving it as unexplained permanent warning noise.
